...
export function syncGuardViolations({
  catalogCount,
  activeCount,
  removedCount,
  priceChangePercents = [],
  ...
  const violations = [];
  if (catalogCount > 0 && activeCount === 0) {
    violations.push('eBay returned zero active listings');
  }
  if (removedCount > maxRemovals) {
    violations.push(`${removedCount} listings would be removed (limit ${maxRemovals})`);
  }
  const largestPriceChange = Math.max(0, ...priceChangePercents.filter(Number.isFinite));
  if (largestPriceChange > maxPriceChangePercent) {
    violations.push(`a price would change ${largestPriceChange.toFixed(1)}% (limit ${maxPriceChangePercent}%)`);
  }
  return violations;
}
