...
import { catalog, directPriceCents, formatMoney, maxQuantity, priceLookupKey } from '../../lib/store-catalog.mjs';
...
export default async (request) => {
  ...
  try {
    ...
    return json({
      ...
      updated,
      pricesCreated,
      archived: staleProducts.length,
      directPrices: catalog.map((item) => ({ listingId: item.id, amount: formatMoney(directPriceCents(item)) }))
    });
  } catch (error) {
