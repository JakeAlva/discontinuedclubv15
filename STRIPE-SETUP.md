...
Run `npm run stripe:catalog -- --apply` with a test key to create or update the Stripe Products and one-time Prices. The script identifies products by eBay listing ID, so rerunning it updates the same catalog instead of duplicating products. It does not touch unrelated Stripe products.
