...
2. Confirm the current shipping policy: USPS Ground Advantage is $7.49 below a $100 item subtotal and free at $100 or more.
3. Decide whether Stripe Tax should be enabled based on the states where the business is registered to collect sales tax.
...
Shipping is calculated by the trusted checkout function from the cart's item subtotal. It charges $7.49 below $100 and creates a free shipping option at $100 or more, so no reusable Stripe Shipping Rate is required.
...
Stripe does not automatically reduce an eBay listing's quantity. Until an inventory database and eBay API sync are added, every direct order must trigger an immediate manual quantity update or listing end on eBay. Checkout limits mirror the current eBay quantities, with one as the default.
